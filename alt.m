...
Alt5_ListGroups := function (p, P, O, M, L: Sparse := true)
   ...
   for i in [1..#L] do
      ...
      u := GL(5, F)!u; 
      c := GL(5, F)!c; 
      S := sub<GL(5, F) | Generators (M[j]), s, u * c>;
      Append (~T, S);
   end for;
   return T;
end function;
...
Alt_ListGroups := function (p, P, M, L)
   T := []; NP := [];
   s := Def_s (p);
   u := Def_u (p);
   for i in L do
...
Alt_Reps := function (p, m)
   ...
   X, P := Alt_ListGroups (p, P, M, L);
   index := EliminateReducible (p, P, O: offset := 0);
   X := [X[i]: i in index];
   if p eq 5 then 
      Y := Alt5_ListGroups (p, P, O, M, L); 
   else 
      Y := []; 
   end if;
   // if p eq 5 then "# of special groups is ", #Y; end if;
   return X cat Y;
end function;
