...
Weights := function (b, n)
   if b eq 1 then 
     ...
     B := [Eltseq (x): x in Basis (V)];
     Z := Integers ();
     R := [[Z!y :y  in x]: x in B];
     assert forall{x: x in R | &+x eq b};
     return R;
   end if;
   l := [0..b];
   C := CartesianProduct ([l: i in [1..n]]);
"Cardinality of C is ", #C;
   R := [];
   for x in C do
      y := [y: y in x];
      if &+y eq b then Append (~R, y); end if;
   end for;
   return R;
end function;
...
Apply_gamma := function (a)
   ...
   C := BaseRing (a);
   return GL(p, C) ! DiagonalMatrix (C, block);
end function;
...
Apply_delta := function (a)
   ...
   r := &*[a[i,i]: i in [1..p]];
   C := BaseRing (a);
   return GL(p, C) ! ScalarMatrix (C, p, r);
end function;
...
SModule := function (U: Group := true) 
   if Type (U) eq GrpMatElt then U := [U]; end if;
   z := Rep (U);
   P := Generic (Parent (z));
   p := Degree (P);
   F := BaseRing (P);
   ...
   if Group then 
      ...
      return sub<MatrixAlgebra (F, p) | [s^-i * z * s^i: i in [1..p - 1]]>;
   end if;
end function;
...
Def_Y := function (p, j, k, l: Check := false)
   ...
   if j eq 0 and l eq 0 then 
      ...
      a := GL(p, F) ! Eltseq (x1) * GL(p, F) ! Eltseq (zl); 
      H := sub<GL (p, F) | a, [X.i: i in [1..Ngens (X)]], z2>;
      return H;
   else
      "Invalid parameters";
      return false;
...
SetupGroups := function (p, C: Sparse := true)
   L := [ ];
nmr := 0;
   for c in C do
nmr +:=1;
      lcm := LCM ([CyclotomicOrder (BaseRing (c[i])): i in [1..#c]]);
      "SetupGroups: # and LCM is ", nmr, lcm;
      if lcm lt 2^30 then 
...
ProcessOrder := function (p, q, M: Sparse := true) 
   X := [];
   s := (p - 1) div Order (q, p);
   W := Weights (M, s);
   for w in W do 
...
ConstructModules := function (p, O)
   ...
   // Sylow p-subgroup 
   Y, M := ConstructSylowp (p, v);
   // "#Y is ", #Y;

   for i in [1..#D] do
      // "ConstructModules i = ", i, "order of p'-dash scalars = ", D[i];

      // coprime q-groups 
       "Coprime subgroups";
      Q, P := ConstructCoprime (p, D[i]);
      // "i Order ", i, D[i], [#x: x in Q];

      if D[i] gt 1 and #Q eq 0 then continue; end if;
      // "Q order is ", D[i];
      //

      // "scalar matrix";
      // p-dash scalars 
      Z := ConstructScalar (p, O div (D[i] * p^v));
      // "#Z is ", O div (D[i] * p^v);
      z := O div (D[i] * p^v);

      if #Q eq 0 and #Y eq 0 then 
         // "Case 1";
         A := Z;
         P := [[[], []]];
      elif #Q eq 0 then 
...
MyIsAbelian := function (M)
   gens := Generators (M);
   return forall{<x, y>: x in gens, y in gens | x * y eq y * x}; 
end function;
...
// with projection <s>
IrreducibleGroups := function (p, O)
   L, P := ConstructModules (p, O);
