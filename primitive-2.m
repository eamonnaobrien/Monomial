import "s-modules.m": Def_x_pj, Def_z;

// irreducible primitive groups of order m up to conjugacy in GL(2, C) 

PrimitiveDimension2 := function (m: Soluble := true, Insoluble := true)
   if (m mod 12 ne 0) or IsOdd (m div 12) then return []; end if;
...
"HERE ", Soluble;
   ...
   n := m div 24;
   if Soluble and m mod 24 eq 0 and IsEven (n) then 
   ...
   n := m div 60;
   if Insoluble and m mod 60 eq 0 and IsEven (n) then 
