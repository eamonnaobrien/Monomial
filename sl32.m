...
SL32_ListGroups := function (p, P, M, L, S, V, N_S, N_V: Sparse := true)
   ...
   for i in [1..#V] do 
      ...
      v := GL(p, F) ! v;
      f := GL(p, F) ! f;
      G := sub<GL(p, F) | Generators (M[j]), s, v * f>;
      Append (~T, G);
   end for;
   ...
   for i in [1..#S] do 
      ...
      v := GL(p, F) ! v;
      g := GL(p, F) ! g;
      G := sub<GL(p, F) | Generators (M[j]), s, v * g>;
      Append (~T, G);
   end for;
...
SL32_Reps := function (p, m) 
   ...
   V0, P, Y := SL32_ListGroups (p, P, M, L, S, V, N_S, N_V: Sparse := true);
